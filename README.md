...
- **知识库问答** — 基于 Qdrant 向量检索，覆盖院校、专业、政策规则、省份数据、风格案例五大维度
- **权威数据搜索** — 基于 Tavily/Brave 实时搜索教育考试院、高校招生网等权威来源，获取最新录取分数、位次、招生计划
...
- **明确区分投档线和专业线**，不拿全校最低分误导用户
- **不编造数据**，多源交叉验证，标注来源可靠性
- **对普通家庭**，优先讲投入产出比
