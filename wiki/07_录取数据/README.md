...
> 当前已收录 87 万+ 条专业录取线、2.7 万条院校录取线、94 条控制线。
...
> 脚本已迁移至项目根目录的 `sqlite/`，数据库文件在 `data/gaokao_2025.db`。

### 1. 初始化数据库

```bash
bun sqlite/init_db.js
```

输出 `data/gaokao_2025.db` 数据库文件，包含：
- 31 省控制线（批次线）
...
```bash
# 查某省控制线
bun sqlite/query.js --lines --province 山东

# 查某校在某省的录取线
bun sqlite/query.js --school 清华大学 --province 山东

# 查某校在所有省的录取线
bun sqlite/query.js --school 清华大学

# 查某省某专业各校录取线
bun sqlite/query.js --major 计算机 --province 山东

# 查某校某专业录取线
bun sqlite/query.js --school 清华大学 --major 计算机
```
...
```bash
# 导入院校录取线
bun sqlite/import_csv.js --type school --file ./data/xxx.csv

# 导入专业录取线
bun sqlite/import_csv.js --type major --file ./data/xxx.csv

# 导入控制线
bun sqlite/import_csv.js --type lines --file ./data/xxx.csv
```
...
│   └── fetch_baidu_gaokao.js ← 百度高考数据爬虫
├── data/
│   └── gaokao_2025.db      ← SQLite 数据库
└── wiki/07_录取数据/
    ├── README.md           ← 本文件
    ├── 数据索引与查询路径.md
    ├── AI查数与数据使用规则.md
    └── 数据检查报告_2026-04-12.md
```
